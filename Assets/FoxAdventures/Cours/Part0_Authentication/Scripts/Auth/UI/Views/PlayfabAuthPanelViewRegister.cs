...
public class PlayfabAuthPanelViewRegister : PlayfabAuthPanelView
{
    ...
    private void TryRegister()
    {
        ...
        if (string.IsNullOrWhiteSpace(email) == false && string.IsNullOrWhiteSpace(password) == false)
        {
            ...
            ////if (remember == true)
            //{
            //    PlayerPrefs.SetString(PlayfabAuth.PlayfabAuthPlayerPrefsKeyUsername, username);
    ...
    private void OnRegisterSuccess(RegisterPlayFabUserResult registerPlayFabUserResult)
    {
        // Log
        Debug.LogWarning("PlayfabAuthPanelViewRegister.OnRegisterSuccess() - " + registerPlayFabUserResult.ToString());

        // Show login view
        if (this.PlayfabAuthPanel != null)
            this.PlayfabAuthPanel.ShowLogin();
    }
    private void OnRegisterError(PlayFabError registerError)
    {
        Debug.LogError("PlayfabAuthPanelViewRegister.OnRegisterError() - Error: " + registerError.ToString());
        //if (registerError != null && (registerError.Error == PlayFabErrorCode.ValueAlreadyExists || registerError.Error == PlayFabErrorCode.NameNotAvailable))
        //{
        //    // Show login view
        //    if (this.PlayfabAuthPanel != null)
        //        this.PlayfabAuthPanel.ShowLogin();
