...
public class Leaderboard : MonoBehaviour
{
    ...
    public void RefreshLeaderboard()
    {
        ...
        if (PlayFab.PlayFabAuthenticationAPI.IsEntityLoggedIn() == true)
        {
            PlayFabClientAPI.GetLeaderboard(new GetLeaderboardRequest()
            {
                ...
                StartPosition = 0,
                MaxResultsCount = 10,
            }, this.OnGetLeaderboardSuccess, this.OnGetLeaderboardError);
        }
    }

    private void OnGetLeaderboardSuccess(GetLeaderboardResult getLeaderboardResult)
    {
        ...
        {
            for (int i=0; i < getLeaderboardResult.Leaderboard.Count; i++)
            {
                PlayerLeaderboardEntry playerLeaderboardEntry = getLeaderboardResult.Leaderboard[i];
                if (playerLeaderboardEntry != null)
                {
                    // Get data
                    string username = playerLeaderboardEntry.DisplayName;
                    int statValue = playerLeaderboardEntry.StatValue;
    ...
    private void OnGetLeaderboardError(PlayFabError playFabError)
    {
        Debug.LogError("Leaderboard.OnGetLeaderboardError() - Error: " + playFabError.ToString());
    }
